/*
 * OpenCL GPU-accelerated Adaptive EMA with Volume Strategy v1 Optimizer
 * Usage: ./optimize <TICKER> <INTERVAL>
 * Example: ./optimize QQQ 1h
 * 
 * Compilation:
 *   make STRATEGY=adaptive_ema_vol_v1
 */
...
#define STRATEGY_NAME "adaptive_ema_vol_v1"
...
typedef struct {
    ...
    int rsi_trending_min_min, rsi_trending_min_max;
    int rsi_trending_max_min, rsi_trending_max_max;
} Config;
...
void load_config(const char* interval, Config* config) {
    if (strcmp(interval, "1h") == 0) {
            ...
            int def_rsi_length = RSI_LENGTH_1H;
            int def_rsi_trending_min = RSI_TRENDING_MIN_1H;
            int def_rsi_trending_max = RSI_TRENDING_MAX_1H;
            ...
            config->rsi_length_min = (int)(def_rsi_length * (1 - SEARCH_PERCENT_RSI_LENGTH_1H));
            config->rsi_length_max = (int)(def_rsi_length * (1 + SEARCH_PERCENT_RSI_LENGTH_1H));
            config->rsi_trending_min_min = (int)(def_rsi_trending_min * (1 - SEARCH_PERCENT_RSI_TRENDING_MIN_1H));
            config->rsi_trending_min_max = (int)(def_rsi_trending_min * (1 + SEARCH_PERCENT_RSI_TRENDING_MIN_1H));
            config->rsi_trending_max_min = (int)(def_rsi_trending_max * (1 - SEARCH_PERCENT_RSI_TRENDING_MAX_1H));
            config->rsi_trending_max_max = (int)(def_rsi_trending_max * (1 + SEARCH_PERCENT_RSI_TRENDING_MAX_1H));
        #else
            ...
            int def_rsi_length = RSI_LENGTH_4H;
            int def_rsi_trending_min = RSI_TRENDING_MIN_4H;
            int def_rsi_trending_max = RSI_TRENDING_MAX_4H;
            ...
            config->rsi_length_min = (int)(def_rsi_length * (1 - SEARCH_PERCENT_RSI_LENGTH_4H));
            config->rsi_length_max = (int)(def_rsi_length * (1 + SEARCH_PERCENT_RSI_LENGTH_4H));
            config->rsi_trending_min_min = (int)(def_rsi_trending_min * (1 - SEARCH_PERCENT_RSI_TRENDING_MIN_4H));
            config->rsi_trending_min_max = (int)(def_rsi_trending_min * (1 + SEARCH_PERCENT_RSI_TRENDING_MIN_4H));
            config->rsi_trending_max_min = (int)(def_rsi_trending_max * (1 - SEARCH_PERCENT_RSI_TRENDING_MAX_4H));
            config->rsi_trending_max_max = (int)(def_rsi_trending_max * (1 + SEARCH_PERCENT_RSI_TRENDING_MAX_4H));
        #else
            ...
            int def_rsi_length = RSI_LENGTH_1D;
            int def_rsi_trending_min = RSI_TRENDING_MIN_1D;
            int def_rsi_trending_max = RSI_TRENDING_MAX_1D;
            ...
            config->rsi_length_min = (int)(def_rsi_length * (1 - SEARCH_PERCENT_RSI_LENGTH_1D));
            config->rsi_length_max = (int)(def_rsi_length * (1 + SEARCH_PERCENT_RSI_LENGTH_1D));
            config->rsi_trending_min_min = (int)(def_rsi_trending_min * (1 - SEARCH_PERCENT_RSI_TRENDING_MIN_1D));
            config->rsi_trending_min_max = (int)(def_rsi_trending_min * (1 + SEARCH_PERCENT_RSI_TRENDING_MIN_1D));
            config->rsi_trending_max_min = (int)(def_rsi_trending_max * (1 - SEARCH_PERCENT_RSI_TRENDING_MAX_1D));
            config->rsi_trending_max_max = (int)(def_rsi_trending_max * (1 + SEARCH_PERCENT_RSI_TRENDING_MAX_1D));
        #else
...
void export_results_to_json(const char* ticker, const char* interval, const char* strategy,
    ...
    fprintf(json_file, "    \"rsi_trending_min\": %.0f,\n", best_params[9]);
    fprintf(json_file, "    \"rsi_trending_max\": %.0f\n", best_params[10]);
    fprintf(json_file, "  },\n");
...
// Load CSV data
int load_csv(const char* filename, float** closes, float** highs, float** lows, float** volumes, long long** timestamps) {
    ...
    *volumes = malloc(count * sizeof(float));
...
int main(int argc, char** argv) {
    ...
    printf("   Ticker: %s\n", ticker);
    printf("   Interval: %s\n", interval);
    printf("   Strategy: %s (Adaptive EMA + Volume + Regime Detection)\n", STRATEGY_NAME);
    ...
    int num_candles = load_csv(filename, &closes, &highs, &lows, &volumes, &timestamps);
    ...
    printf("============================================================\n");
    printf("🔧 Optimizing %s - Adaptive EMA Vol v1 (Volume + Regime)\n", ticker);
    printf("============================================================\n\n");
    ...
    int rsi_min_range = config.rsi_trending_min_max - config.rsi_trending_min_min + 1;
    int rsi_max_range = config.rsi_trending_max_max - config.rsi_trending_max_min + 1;
    printf("     RSI Length: %d-%d (%d values)\n", config.rsi_length_min, config.rsi_length_max, rsi_len_range);
    printf("     RSI Trending Min: %d-%d (%d values)\n", config.rsi_trending_min_min, config.rsi_trending_min_max, rsi_min_range);
    printf("     RSI Trending Max: %d-%d (%d values)\n", config.rsi_trending_max_min, config.rsi_trending_max_max, rsi_max_range);
                            ...
                            for (int adx_len = config.adx_length_min; adx_len <= config.adx_length_max; adx_len++) {
                                for (float adx_thresh = config.adx_threshold_min; adx_thresh <= config.adx_threshold_max; adx_thresh += 1.0f) {
                                    for (int rsi_len = config.rsi_length_min; rsi_len <= config.rsi_length_max; rsi_len++) {
                                        for (int rsi_min = config.rsi_trending_min_min; rsi_min <= config.rsi_trending_min_max; rsi_min++) {
                                            for (int rsi_max = config.rsi_trending_max_min; rsi_max <= config.rsi_trending_max_max; rsi_max++) {
                                                if (rsi_min < rsi_max) {
                                                    num_combinations++;
                                                }
                                            }
    ...
    if (num_combinations > MAX_COMBINATIONS) {
        ...
        free(closes); free(highs); free(lows); free(volumes); free(timestamps); free(kernel_source);
        return 1;
    }
    ...
    float* h_params = malloc(num_combinations * 11 * sizeof(float));
    int idx = 0;
                                    ...
                                    for (int rsi_len = config.rsi_length_min; rsi_len <= config.rsi_length_max; rsi_len++) {
                                        for (int rsi_min = config.rsi_trending_min_min; rsi_min <= config.rsi_trending_min_max; rsi_min++) {
                                            for (int rsi_max = config.rsi_trending_max_min; rsi_max <= config.rsi_trending_max_max; rsi_max++) {
                                                ...
                                                h_params[idx * 11 + 0] = fb;
                                                h_params[idx * 11 + 1] = sb;
                                                h_params[idx * 11 + 2] = fm;
                                                h_params[idx * 11 + 3] = sm;
                                                h_params[idx * 11 + 4] = atr;
                                                h_params[idx * 11 + 5] = vt;
                                                h_params[idx * 11 + 6] = adx_len;
                                                h_params[idx * 11 + 7] = adx_thresh;
                                                h_params[idx * 11 + 8] = rsi_len;
                                                h_params[idx * 11 + 9] = rsi_min;
                                                h_params[idx * 11 + 10] = rsi_max;
                                                idx++;
                                            }
                                        }
                                    }
    ...
    cl_mem d_volumes = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                      num_candles * sizeof(float), volumes, &err);
    cl_mem d_params = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     num_combinations * 11 * sizeof(float), h_params, &err);
    ...
    clSetKernelArg(kernel, 3, sizeof(cl_mem), &d_volumes);
    clSetKernelArg(kernel, 4, sizeof(int), &num_candles);
    clSetKernelArg(kernel, 5, sizeof(cl_mem), &d_params);
    clSetKernelArg(kernel, 6, sizeof(cl_mem), &d_results);
    clSetKernelArg(kernel, 7, sizeof(int), &num_combinations);
    clSetKernelArg(kernel, 8, sizeof(cl_mem), &d_trade_log);
    ...
    if (best_idx >= 0) {
        ...
        printf("   Fast Base: %.0f\n", h_params[best_idx * 11 + 0]);
        printf("   Slow Base: %.0f\n", h_params[best_idx * 11 + 1]);
        printf("   Fast Mult: %.2f\n", h_params[best_idx * 11 + 2]);
        printf("   Slow Mult: %.2f\n", h_params[best_idx * 11 + 3]);
        printf("   ATR Length: %.0f\n", h_params[best_idx * 11 + 4]);
        printf("   Vol Threshold: %.0f%%\n", h_params[best_idx * 11 + 5]);
        printf("   ADX Length: %.0f\n", h_params[best_idx * 11 + 6]);
        printf("   ADX Threshold: %.2f\n", h_params[best_idx * 11 + 7]);
        printf("   RSI Length: %.0f\n", h_params[best_idx * 11 + 8]);
        printf("   RSI Trending Min: %.0f\n", h_params[best_idx * 11 + 9]);
        printf("   RSI Trending Max: %.0f\n", h_params[best_idx * 11 + 10]);
    }
    ...
    if (best_idx >= 0) {
        ...
        float best_params_only[11];
        for (int i = 0; i < 11; i++) best_params_only[i] = h_params[best_idx * 11 + i];
        
        memset(h_trade_log, 0, 300 * sizeof(float));
        
        cl_mem d_best_params = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                              11 * sizeof(float), best_params_only, &err);
