...
int main(int argc, char** argv) {
    ...
    // Allocate parameter array (6 params per combination)
    float* h_params = malloc(num_combinations * 6 * sizeof(float));
    int idx = 0;
    ...
    cl_mem d_params = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     num_combinations * 6 * sizeof(float), h_params, &err);
    ...
    if (best_idx >= 0) {
        ...
        printf("   Fast Base: %.0f\n", h_params[best_idx * 6 + 0]);
        printf("   Slow Base: %.0f\n", h_params[best_idx * 6 + 1]);
        printf("   Fast Mult: %.2f\n", h_params[best_idx * 6 + 2]);
        printf("   Slow Mult: %.2f\n", h_params[best_idx * 6 + 3]);
        printf("   ATR Length: %.0f\n", h_params[best_idx * 6 + 4]);
        printf("   Vol Threshold: %.0f%%\n", h_params[best_idx * 6 + 5]);
    }
