/*
 * OpenCL GPU-accelerated Adaptive EMA Strategy v2 Optimizer
 * Usage: ./optimize <TICKER> <INTERVAL>
 * Example: ./optimize QQQ 1h
 * 
 * Compilation:
 *   make STRATEGY=adaptive_ema_v4
 */
...
#define STRATEGY_NAME "adaptive_ema_v4"
...
typedef struct {
    int ema_fast_base_min, ema_fast_base_max;
    int ema_slow_base_min, ema_slow_base_max;
    int adx_length_min, adx_length_max;
    float adx_threshold_min, adx_threshold_max;
    int rsi_length_min, rsi_length_max;
    float rsi_buy_max_min, rsi_buy_max_max;
    float rsi_sell_min_min, rsi_sell_min_max;
    int efficiency_lookback_min, efficiency_lookback_max;
} Config;
...
void load_config(const char* interval, Config* config) {
    if (strcmp(interval, "1h") == 0) {
            ...
            int def_ema_fast = EMA_FAST_BASE_1H;
            int def_ema_slow = EMA_SLOW_BASE_1H;
            int def_adx_len = ADX_LENGTH_1H;
            float def_adx_thresh = ADX_THRESHOLD_1H;
            int def_rsi_len = RSI_LENGTH_1H;
            float def_rsi_buy = RSI_BUY_MAX_1H;
            float def_rsi_sell = RSI_SELL_MIN_1H;
            int def_eff_look = EFFICIENCY_LOOKBACK_1H;
            
            config->ema_fast_base_min = (int)(def_ema_fast * (1 - SEARCH_PERCENT_EMA_FAST_BASE_1H));
            config->ema_fast_base_max = (int)(def_ema_fast * (1 + SEARCH_PERCENT_EMA_FAST_BASE_1H));
            config->ema_slow_base_min = (int)(def_ema_slow * (1 - SEARCH_PERCENT_EMA_SLOW_BASE_1H));
            config->ema_slow_base_max = (int)(def_ema_slow * (1 + SEARCH_PERCENT_EMA_SLOW_BASE_1H));
            config->adx_length_min = (int)(def_adx_len * (1 - SEARCH_PERCENT_ADX_LENGTH_1H));
            config->adx_length_max = (int)(def_adx_len * (1 + SEARCH_PERCENT_ADX_LENGTH_1H));
            ...
            config->rsi_length_min = (int)(def_rsi_len * (1 - SEARCH_PERCENT_RSI_LENGTH_1H));
            config->rsi_length_max = (int)(def_rsi_len * (1 + SEARCH_PERCENT_RSI_LENGTH_1H));
            config->rsi_buy_max_min = def_rsi_buy * (1 - SEARCH_PERCENT_RSI_BUY_MAX_1H);
            config->rsi_buy_max_max = def_rsi_buy * (1 + SEARCH_PERCENT_RSI_BUY_MAX_1H);
            config->rsi_sell_min_min = def_rsi_sell * (1 - SEARCH_PERCENT_RSI_SELL_MIN_1H);
            config->rsi_sell_min_max = def_rsi_sell * (1 + SEARCH_PERCENT_RSI_SELL_MIN_1H);
            config->efficiency_lookback_min = (int)(def_eff_look * (1 - SEARCH_PERCENT_EFFICIENCY_LOOKBACK_1H));
            config->efficiency_lookback_max = (int)(def_eff_look * (1 + SEARCH_PERCENT_EFFICIENCY_LOOKBACK_1H));
        #else
            ...
            int def_ema_fast = EMA_FAST_BASE_4H;
            int def_ema_slow = EMA_SLOW_BASE_4H;
            int def_adx_len = ADX_LENGTH_4H;
            float def_adx_thresh = ADX_THRESHOLD_4H;
            int def_rsi_len = RSI_LENGTH_4H;
            float def_rsi_buy = RSI_BUY_MAX_4H;
            float def_rsi_sell = RSI_SELL_MIN_4H;
            int def_eff_look = EFFICIENCY_LOOKBACK_4H;
            
            config->ema_fast_base_min = (int)(def_ema_fast * (1 - SEARCH_PERCENT_EMA_FAST_BASE_4H));
            config->ema_fast_base_max = (int)(def_ema_fast * (1 + SEARCH_PERCENT_EMA_FAST_BASE_4H));
            config->ema_slow_base_min = (int)(def_ema_slow * (1 - SEARCH_PERCENT_EMA_SLOW_BASE_4H));
            config->ema_slow_base_max = (int)(def_ema_slow * (1 + SEARCH_PERCENT_EMA_SLOW_BASE_4H));
            config->adx_length_min = (int)(def_adx_len * (1 - SEARCH_PERCENT_ADX_LENGTH_4H));
            config->adx_length_max = (int)(def_adx_len * (1 + SEARCH_PERCENT_ADX_LENGTH_4H));
            ...
            config->rsi_length_min = (int)(def_rsi_len * (1 - SEARCH_PERCENT_RSI_LENGTH_4H));
            config->rsi_length_max = (int)(def_rsi_len * (1 + SEARCH_PERCENT_RSI_LENGTH_4H));
            config->rsi_buy_max_min = def_rsi_buy * (1 - SEARCH_PERCENT_RSI_BUY_MAX_4H);
            config->rsi_buy_max_max = def_rsi_buy * (1 + SEARCH_PERCENT_RSI_BUY_MAX_4H);
            config->rsi_sell_min_min = def_rsi_sell * (1 - SEARCH_PERCENT_RSI_SELL_MIN_4H);
            config->rsi_sell_min_max = def_rsi_sell * (1 + SEARCH_PERCENT_RSI_SELL_MIN_4H);
            config->efficiency_lookback_min = (int)(def_eff_look * (1 - SEARCH_PERCENT_EFFICIENCY_LOOKBACK_4H));
            config->efficiency_lookback_max = (int)(def_eff_look * (1 + SEARCH_PERCENT_EFFICIENCY_LOOKBACK_4H));
        #else
            ...
            int def_ema_fast = EMA_FAST_BASE_1D;
            int def_ema_slow = EMA_SLOW_BASE_1D;
            int def_adx_len = ADX_LENGTH_1D;
            float def_adx_thresh = ADX_THRESHOLD_1D;
            int def_rsi_len = RSI_LENGTH_1D;
            float def_rsi_buy = RSI_BUY_MAX_1D;
            float def_rsi_sell = RSI_SELL_MIN_1D;
            int def_eff_look = EFFICIENCY_LOOKBACK_1D;
            
            config->ema_fast_base_min = (int)(def_ema_fast * (1 - SEARCH_PERCENT_EMA_FAST_BASE_1D));
            config->ema_fast_base_max = (int)(def_ema_fast * (1 + SEARCH_PERCENT_EMA_FAST_BASE_1D));
            config->ema_slow_base_min = (int)(def_ema_slow * (1 - SEARCH_PERCENT_EMA_SLOW_BASE_1D));
            config->ema_slow_base_max = (int)(def_ema_slow * (1 + SEARCH_PERCENT_EMA_SLOW_BASE_1D));
            config->adx_length_min = (int)(def_adx_len * (1 - SEARCH_PERCENT_ADX_LENGTH_1D));
            config->adx_length_max = (int)(def_adx_len * (1 + SEARCH_PERCENT_ADX_LENGTH_1D));
            ...
            config->rsi_length_min = (int)(def_rsi_len * (1 - SEARCH_PERCENT_RSI_LENGTH_1D));
            config->rsi_length_max = (int)(def_rsi_len * (1 + SEARCH_PERCENT_RSI_LENGTH_1D));
            config->rsi_buy_max_min = def_rsi_buy * (1 - SEARCH_PERCENT_RSI_BUY_MAX_1D);
            config->rsi_buy_max_max = def_rsi_buy * (1 + SEARCH_PERCENT_RSI_BUY_MAX_1D);
            config->rsi_sell_min_min = def_rsi_sell * (1 - SEARCH_PERCENT_RSI_SELL_MIN_1D);
            config->rsi_sell_min_max = def_rsi_sell * (1 + SEARCH_PERCENT_RSI_SELL_MIN_1D);
            config->efficiency_lookback_min = (int)(def_eff_look * (1 - SEARCH_PERCENT_EFFICIENCY_LOOKBACK_1D));
            config->efficiency_lookback_max = (int)(def_eff_look * (1 + SEARCH_PERCENT_EFFICIENCY_LOOKBACK_1D));
        #else
...
void export_results_to_json(const char* ticker, const char* interval, const char* strategy,
    ...
    fprintf(json_file, "    \"ema_fast_base\": %.0f,\n", best_params[0]);
    fprintf(json_file, "    \"ema_slow_base\": %.0f,\n", best_params[1]);
    fprintf(json_file, "    \"adx_length\": %.0f,\n", best_params[2]);
    fprintf(json_file, "    \"adx_threshold\": %.1f,\n", best_params[3]);
    fprintf(json_file, "    \"rsi_length\": %.0f,\n", best_params[4]);
    fprintf(json_file, "    \"rsi_buy_max\": %.1f,\n", best_params[5]);
    fprintf(json_file, "    \"rsi_sell_min\": %.1f,\n", best_params[6]);
    fprintf(json_file, "    \"efficiency_lookback\": %.0f\n", best_params[7]);
    fprintf(json_file, "  },\n");
...
int main(int argc, char** argv) {
    ...
    printf("   Ticker: %s\n", ticker);
    printf("   Interval: %s\n", interval);
    printf("   Strategy: %s (KAMA+ADX+RSI Adaptive EMA)\n", STRATEGY_NAME);
    ...
    printf("============================================================\n");
    printf("🔧 Optimizing %s - Adaptive EMA v4\n", ticker);
    printf("============================================================\n\n");
    printf("⚡ Calculating parameter space size...\n");
    
    // Calculate ranges
    int efb_range = config.ema_fast_base_max - config.ema_fast_base_min + 1;
    int esb_range = config.ema_slow_base_max - config.ema_slow_base_min + 1;
    int adx_len_range = config.adx_length_max - config.adx_length_min + 1;
    int rsi_len_range = config.rsi_length_max - config.rsi_length_min + 1;
    int eff_range = config.efficiency_lookback_max - config.efficiency_lookback_min + 1;
    
    // For float parameters, sample at reasonable increments
    int adx_thresh_steps = (int)((config.adx_threshold_max - config.adx_threshold_min) / 2.0f) + 1;
    int rsi_buy_steps = (int)((config.rsi_buy_max_max - config.rsi_buy_max_min) / 5.0f) + 1;
    int rsi_sell_steps = (int)((config.rsi_sell_min_max - config.rsi_sell_min_min) / 3.0f) + 1;
    
    printf("   Parameter ranges:\n");
    printf("     EMA Fast Base: %d-%d (%d values)\n", config.ema_fast_base_min, config.ema_fast_base_max, efb_range);
    printf("     EMA Slow Base: %d-%d (%d values)\n", config.ema_slow_base_min, config.ema_slow_base_max, esb_range);
    printf("     ADX Length: %d-%d (%d values)\n", config.adx_length_min, config.adx_length_max, adx_len_range);
    printf("     ADX Threshold: %.1f-%.1f (%d values)\n", config.adx_threshold_min, config.adx_threshold_max, adx_thresh_steps);
    printf("     RSI Length: %d-%d (%d values)\n", config.rsi_length_min, config.rsi_length_max, rsi_len_range);
    printf("     RSI Buy Max: %.1f-%.1f (%d values)\n", config.rsi_buy_max_min, config.rsi_buy_max_max, rsi_buy_steps);
    printf("     RSI Sell Min: %.1f-%.1f (%d values)\n", config.rsi_sell_min_min, config.rsi_sell_min_max, rsi_sell_steps);
    printf("     Efficiency Lookback: %d-%d (%d values)\n", config.efficiency_lookback_min, config.efficiency_lookback_max, eff_range);
    
    // Count valid combinations (ema_fast_base < ema_slow_base)
    int num_combinations = 0;
    for (int efb = config.ema_fast_base_min; efb <= config.ema_fast_base_max; efb++) {
        for (int esb = config.ema_slow_base_min; esb <= config.ema_slow_base_max; esb++) {
            if (efb >= esb) continue;
            for (int adx_len = config.adx_length_min; adx_len <= config.adx_length_max; adx_len++) {
                for (float adx_thresh = config.adx_threshold_min; adx_thresh <= config.adx_threshold_max; adx_thresh += 2.0f) {
                    for (int rsi_len = config.rsi_length_min; rsi_len <= config.rsi_length_max; rsi_len++) {
                        for (float rsi_buy = config.rsi_buy_max_min; rsi_buy <= config.rsi_buy_max_max; rsi_buy += 5.0f) {
                            for (float rsi_sell = config.rsi_sell_min_min; rsi_sell <= config.rsi_sell_min_max; rsi_sell += 3.0f) {
                                for (int eff = config.efficiency_lookback_min; eff <= config.efficiency_lookback_max; eff++) {
                                    num_combinations++;
                                }
                            }
                        }
                ...
                for (float adx_thresh = config.adx_threshold_min; adx_thresh <= config.adx_threshold_max; adx_thresh += 2.0f) {
                    for (int rsi_len = config.rsi_length_min; rsi_len <= config.rsi_length_max; rsi_len++) {
                        for (float rsi_buy = config.rsi_buy_max_min; rsi_buy <= config.rsi_buy_max_max; rsi_buy += 5.0f) {
                            for (float rsi_sell = config.rsi_sell_min_min; rsi_sell <= config.rsi_sell_min_max; rsi_sell += 3.0f) {
                                for (int eff = config.efficiency_lookback_min; eff <= config.efficiency_lookback_max; eff++) {
                                    h_params[idx * 8 + 0] = efb;
                                    h_params[idx * 8 + 1] = esb;
                                    h_params[idx * 8 + 2] = adx_len;
                                    h_params[idx * 8 + 3] = adx_thresh;
                                    h_params[idx * 8 + 4] = rsi_len;
                                    h_params[idx * 8 + 5] = rsi_buy;
                                    h_params[idx * 8 + 6] = rsi_sell;
                                    h_params[idx * 8 + 7] = eff;
                                    idx++;
                                }
                            }
                        }
