/*
 * OpenCL GPU-accelerated Adaptive Donchian Breakout Strategy v1 Optimizer
...
#define STRATEGY_NAME "adaptive_donchian_v1"
...
typedef struct {
    int donchian_length_min, donchian_length_max;
    int atr_length_min, atr_length_max;
    float atr_multiplier_min, atr_multiplier_max;
...
void load_config(const char* interval, Config* config) {
    if (strcmp(interval, "1h") == 0) {
            ...
            int def_donchian = DONCHIAN_LENGTH_1H;
            int def_atr = ATR_LENGTH_1H;
            float def_atr_mult = ATR_MULTIPLIER_1H;
            int def_adx_length = ADX_LENGTH_1H;
            float def_adx_thresh = ADX_THRESHOLD_1H;
            
            config->donchian_length_min = (int)(def_donchian * (1 - SEARCH_PERCENT_DONCHIAN_LENGTH_1H));
            config->donchian_length_max = (int)(def_donchian * (1 + SEARCH_PERCENT_DONCHIAN_LENGTH_1H));
            config->atr_length_min = (int)(def_atr * (1 - SEARCH_PERCENT_ATR_LENGTH_1H));
            config->atr_length_max = (int)(def_atr * (1 + SEARCH_PERCENT_ATR_LENGTH_1H));
            config->atr_multiplier_min = def_atr_mult * (1 - SEARCH_PERCENT_ATR_MULTIPLIER_1H);
            config->atr_multiplier_max = def_atr_mult * (1 + SEARCH_PERCENT_ATR_MULTIPLIER_1H);
            ...
            int def_donchian = DONCHIAN_LENGTH_4H;
            int def_atr = ATR_LENGTH_4H;
            float def_atr_mult = ATR_MULTIPLIER_4H;
            int def_adx_length = ADX_LENGTH_4H;
            float def_adx_thresh = ADX_THRESHOLD_4H;
            
            config->donchian_length_min = (int)(def_donchian * (1 - SEARCH_PERCENT_DONCHIAN_LENGTH_4H));
            config->donchian_length_max = (int)(def_donchian * (1 + SEARCH_PERCENT_DONCHIAN_LENGTH_4H));
            config->atr_length_min = (int)(def_atr * (1 - SEARCH_PERCENT_ATR_LENGTH_4H));
            config->atr_length_max = (int)(def_atr * (1 + SEARCH_PERCENT_ATR_LENGTH_4H));
            config->atr_multiplier_min = def_atr_mult * (1 - SEARCH_PERCENT_ATR_MULTIPLIER_4H);
            config->atr_multiplier_max = def_atr_mult * (1 + SEARCH_PERCENT_ATR_MULTIPLIER_4H);
            ...
            int def_donchian = DONCHIAN_LENGTH_1D;
            int def_atr = ATR_LENGTH_1D;
            float def_atr_mult = ATR_MULTIPLIER_1D;
            int def_adx_length = ADX_LENGTH_1D;
            float def_adx_thresh = ADX_THRESHOLD_1D;
            
            config->donchian_length_min = (int)(def_donchian * (1 - SEARCH_PERCENT_DONCHIAN_LENGTH_1D));
            config->donchian_length_max = (int)(def_donchian * (1 + SEARCH_PERCENT_DONCHIAN_LENGTH_1D));
            config->atr_length_min = (int)(def_atr * (1 - SEARCH_PERCENT_ATR_LENGTH_1D));
            config->atr_length_max = (int)(def_atr * (1 + SEARCH_PERCENT_ATR_LENGTH_1D));
            config->atr_multiplier_min = def_atr_mult * (1 - SEARCH_PERCENT_ATR_MULTIPLIER_1D);
            config->atr_multiplier_max = def_atr_mult * (1 + SEARCH_PERCENT_ATR_MULTIPLIER_1D);
...
void export_results_to_json(const char* ticker, const char* interval, const char* strategy,
    ...
    fprintf(json_file, "    \"donchian_length\": %.0f,\n", best_params[0]);
    fprintf(json_file, "    \"atr_length\": %.0f,\n", best_params[1]);
    fprintf(json_file, "    \"atr_multiplier\": %.2f,\n", best_params[2]);
...
int main(int argc, char** argv) {
    ...
    printf("   Ticker: %s\n", ticker);
    printf("   Interval: %s\n", interval);
    printf("   Strategy: %s (Donchian Breakout with ATR & ADX)\n", STRATEGY_NAME);
    ...
    printf("============================================================\n");
    printf("🔧 Optimizing %s - Donchian Breakout v1 (with ATR & ADX)\n", ticker);
    printf("============================================================\n\n");
    printf("⚡ Calculating parameter space size...\n");
    
    // Calculate ranges
    int dc_range = config.donchian_length_max - config.donchian_length_min + 1;
    ...
    // For float multipliers, sample at 0.1 increments
    int atr_mult_steps = (int)((config.atr_multiplier_max - config.atr_multiplier_min) / 0.1f) + 1;
    int adx_thresh_steps = (int)((config.adx_threshold_max - config.adx_threshold_min) / 1.0f) + 1;
    
    printf("   Parameter ranges:\n");
    printf("     Donchian Length: %d-%d (%d values)\n", config.donchian_length_min, config.donchian_length_max, dc_range);
    printf("     ATR Length: %d-%d (%d values)\n", config.atr_length_min, config.atr_length_max, atr_range);
    printf("     ATR Multiplier: %.1f-%.1f (%d values)\n", config.atr_multiplier_min, config.atr_multiplier_max, atr_mult_steps);
    ...
    for (int dc = config.donchian_length_min; dc <= config.donchian_length_max; dc++) {
        for (int atr = config.atr_length_min; atr <= config.atr_length_max; atr++) {
            for (float atr_mult = config.atr_multiplier_min; atr_mult <= config.atr_multiplier_max; atr_mult += 0.1f) {
    ...
    // Allocate parameter array (5 params per combination)
    float* h_params = malloc(num_combinations * 5 * sizeof(float));
    int idx = 0;
        ...
        for (int atr = config.atr_length_min; atr <= config.atr_length_max; atr++) {
            for (float atr_mult = config.atr_multiplier_min; atr_mult <= config.atr_multiplier_max; atr_mult += 0.1f) {
                for (int adx_len = config.adx_length_min; adx_len <= config.adx_length_max; adx_len++) {
                    for (float adx_thresh = config.adx_threshold_min; adx_thresh <= config.adx_threshold_max; adx_thresh += 1.0f) {
                        h_params[idx * 5 + 0] = dc;
                        h_params[idx * 5 + 1] = atr;
                        h_params[idx * 5 + 2] = atr_mult;
                        h_params[idx * 5 + 3] = adx_len;
                        h_params[idx * 5 + 4] = adx_thresh;
                        idx++;
                    }
                }
            }
    ...
    cl_mem d_params = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     num_combinations * 5 * sizeof(float), h_params, &err);
    ...
    if (best_idx >= 0) {
        ...
        printf("   Donchian Length: %.0f\n", h_params[best_idx * 5 + 0]);
        printf("   ATR Length: %.0f\n", h_params[best_idx * 5 + 1]);
        printf("   ATR Multiplier: %.2f\n", h_params[best_idx * 5 + 2]);
        printf("   ADX Length: %.0f\n", h_params[best_idx * 5 + 3]);
        printf("   ADX Threshold: %.2f\n", h_params[best_idx * 5 + 4]);
    }
