...
#define STRATEGY_NAME "adaptive_ema_v2.2"
...
typedef struct {
    ...
    int rsi_threshold_min, rsi_threshold_max;
} Config;
...
void load_config(const char* interval, Config* config) {
    if (strcmp(interval, "1h") == 0) {
            ...
            int def_rsi_thresh = RSI_THRESHOLD_1H;
            ...
            config->rsi_threshold_min = (int)(def_rsi_thresh * (1 - SEARCH_PERCENT_RSI_THRESHOLD_1H));
            config->rsi_threshold_max = (int)(def_rsi_thresh * (1 + SEARCH_PERCENT_RSI_THRESHOLD_1H));
        #else
...
int main(int argc, char** argv) {
    ...
    printf("   Ticker: %s\n", ticker);
    printf("   Interval: %s\n", interval);
    printf("   Strategy: %s (Volatility-Adaptive EMA)\n", STRATEGY_NAME);
    ...
    float *closes, *highs, *lows, *volumes;
    ...
    printf("============================================================\n");
    printf("🔧 Optimizing %s - Adaptive EMA v2\n", ticker);
    printf("============================================================\n\n");
    ...
    int rsi_th_range = config.rsi_threshold_max - config.rsi_threshold_min + 1;
    ...
    printf("     RSI Threshold: %d-%d (%d values)\n", config.rsi_threshold_min, config.rsi_threshold_max, rsi_th_range);
                    ...
                    for (int atr = config.atr_length_min; atr <= config.atr_length_max; atr++) {
                        for (int vt = config.vol_threshold_min; vt <= config.vol_threshold_max; vt++) {
                            for (int rsi_len = config.rsi_length_min; rsi_len <= config.rsi_length_max; rsi_len++) {
                                for (int rsi_th = config.rsi_threshold_min; rsi_th <= config.rsi_threshold_max; rsi_th++) {
                                    num_combinations++;
                                }
                            }
                        }
                        ...
                        for (int vt = config.vol_threshold_min; vt <= config.vol_threshold_max; vt++) {
                            for (int rsi_len = config.rsi_length_min; rsi_len <= config.rsi_length_max; rsi_len++) {
                                for (int rsi_th = config.rsi_threshold_min; rsi_th <= config.rsi_threshold_max; rsi_th++) {
                                    ...
                                    h_params[idx * 8 + 5] = vt;
                                    h_params[idx * 8 + 6] = rsi_len;
                                    h_params[idx * 8 + 7] = rsi_th;
                                    idx++;
                                }
                            }
                        }
    ...
    if (best_idx >= 0) {
        ...
        int single_combo = 1;
        clSetKernelArg(kernel, 5, sizeof(cl_mem), &d_best_params);
        clSetKernelArg(kernel, 6, sizeof(cl_mem), &d_best_results);
        clSetKernelArg(kernel, 7, sizeof(int), &single_combo);
        clSetKernelArg(kernel, 8, sizeof(cl_mem), &d_best_trade_log);
    ...
    for (int i = 0; i < 100; i++) {
        ...
        // Break only if we hit an empty entry (all zeros)
        if (candle_idx == 0 && price == 0.0f && is_buy == 0) break;
