/*
 * OpenCL GPU-accelerated Simple EMA Strategy v1 Optimizer
 * Usage: ./optimize <TICKER> <INTERVAL>
 * Example: ./optimize GOOG 1h
 * 
 * Compilation:
 *   make STRATEGY=ema_simple_v1
 */
...
#define STRATEGY_NAME "ema_simple_v1"
...
typedef struct {
    int fast_length_min, fast_length_max;
    int slow_length_min, slow_length_max;
} Config;
...
void load_config(const char* interval, Config* config) {
    if (strcmp(interval, "1h") == 0) {
            ...
            int default_fast = FAST_LENGTH_1H;
            int default_slow = SLOW_LENGTH_1H;
            
            config->fast_length_min = (int)(default_fast * (1 - SEARCH_PERCENT_FAST_1H));
            config->fast_length_max = (int)(default_fast * (1 + SEARCH_PERCENT_FAST_1H));
            config->slow_length_min = (int)(default_slow * (1 - SEARCH_PERCENT_SLOW_1H));
            config->slow_length_max = (int)(default_slow * (1 + SEARCH_PERCENT_SLOW_1H));
        #else
            ...
            int default_fast = FAST_LENGTH_4H;
            int default_slow = SLOW_LENGTH_4H;
            
            config->fast_length_min = (int)(default_fast * (1 - SEARCH_PERCENT_FAST_4H));
            config->fast_length_max = (int)(default_fast * (1 + SEARCH_PERCENT_FAST_4H));
            config->slow_length_min = (int)(default_slow * (1 - SEARCH_PERCENT_SLOW_4H));
            config->slow_length_max = (int)(default_slow * (1 + SEARCH_PERCENT_SLOW_4H));
        #else
            ...
            int default_fast = FAST_LENGTH_1D;
            int default_slow = SLOW_LENGTH_1D;
            
            config->fast_length_min = (int)(default_fast * (1 - SEARCH_PERCENT_FAST_1D));
            config->fast_length_max = (int)(default_fast * (1 + SEARCH_PERCENT_FAST_1D));
            config->slow_length_min = (int)(default_slow * (1 - SEARCH_PERCENT_SLOW_1D));
            config->slow_length_max = (int)(default_slow * (1 + SEARCH_PERCENT_SLOW_1D));
        #else
...
void export_results_to_json(const char* ticker, const char* interval, const char* strategy,
    ...
    fprintf(json_file, "    \"fast_length\": %.0f,\n", best_params[0]);
    fprintf(json_file, "    \"slow_length\": %.0f\n", best_params[1]);
    fprintf(json_file, "  },\n");
...
int main(int argc, char** argv) {
    if (argc < 3 || argc > 4) {
        ...
        printf("\nIntervals: 1h, 4h, 1d\n");
        printf("\nCurrent strategy: %s\n", STRATEGY_NAME);
        printf("To change strategy, recompile with: make STRATEGY=adaptive_ema_v1\n");
        return 1;
    }
    ...
    printf("   Ticker: %s\n", ticker);
    printf("   Interval: %s\n", interval);
    printf("   Strategy: %s (Simple EMA Crossover)\n", STRATEGY_NAME);
    ...
    printf("============================================================\n");
    printf("🔧 Optimizing %s - Simple EMA Search\n", ticker);
    ...
    int fast_range = config.fast_length_max - config.fast_length_min + 1;
    int slow_range = config.slow_length_max - config.slow_length_min + 1;
    
    printf("   Parameter ranges:\n");
    printf("     Fast EMA: %d-%d (%d values)\n", config.fast_length_min, config.fast_length_max, fast_range);
    printf("     Slow EMA: %d-%d (%d values)\n", config.slow_length_min, config.slow_length_max, slow_range);
    
    // Count valid combinations (fast < slow)
    int num_combinations = 0;
    for (int fast = config.fast_length_min; fast <= config.fast_length_max; fast++) {
        for (int slow = config.slow_length_min; slow <= config.slow_length_max; slow++) {
            if (fast < slow) {
                num_combinations++;
            }
        }
    ...
    // Allocate and populate parameter array (2 params per combination)
    float* h_params = malloc(num_combinations * 2 * sizeof(float));
    int idx = 0;
    
    for (int fast = config.fast_length_min; fast <= config.fast_length_max; fast++) {
        for (int slow = config.slow_length_min; slow <= config.slow_length_max; slow++) {
            if (fast < slow) {
                h_params[idx * 2 + 0] = fast;
                h_params[idx * 2 + 1] = slow;
                idx++;
            }
        }
    }
    ...
    cl_mem d_params = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                     num_combinations * 2 * sizeof(float), h_params, &err);
    ...
    if (best_idx >= 0) {
        ...
        printf("   Fast EMA: %.0f\n", h_params[best_idx * 2 + 0]);
        printf("   Slow EMA: %.0f\n", h_params[best_idx * 2 + 1]);
    }
    ...
    // Re-run best parameters to get accurate trade log
    printf("\n============================================================\n");
    ...
    if (best_idx >= 0) {
        ...
        // Create single-parameter buffer
        float best_params_only[2];
        best_params_only[0] = h_params[best_idx * 2 + 0];
        best_params_only[1] = h_params[best_idx * 2 + 1];
        ...
        cl_mem d_best_params = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                                              2 * sizeof(float), best_params_only, &err);
